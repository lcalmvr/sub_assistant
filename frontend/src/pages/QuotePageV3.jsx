        ...
        <div className="lg:col-span-3 border border-gray-200 rounded-lg bg-white p-4">
          ...
          <div className="relative">
            ...
            <div className="pl-6 space-y-1">
              {/* Show excess layers above ours */}
              {isExcess && tower.length > 1 && (() => {
                const sortedTower = [...tower].sort((a, b) => (b.attachment || 0) - (a.attachment || 0));
                const cmaiLayerIdx = sortedTower.findIndex(l => l.carrier?.toUpperCase().includes('CMAI'));
                const layersAbove = sortedTower.slice(0, cmaiLayerIdx);
                return layersAbove.map((layer, idx) => (
              ...
              {/* Our Layer */}
              <div className="bg-purple-600 text-white rounded py-3 px-4 text-center shadow-md">
                <div className="text-[10px] uppercase tracking-wide opacity-80">Our Layer</div>
                <div className="text-lg font-bold">{formatCompact(ourLimit)}</div>
                {isExcess && attachment > 0 && (
                  <div className="text-xs opacity-80">xs {formatCompact(attachment)}</div>
                )}
              </div>
              ...
              {isExcess && tower.length > 1 && (() => {
                const sortedTower = [...tower].sort((a, b) => (b.attachment || 0) - (a.attachment || 0));
                const cmaiLayerIdx = sortedTower.findIndex(l => l.carrier?.toUpperCase().includes('CMAI'));
                const layersBelow = sortedTower.slice(cmaiLayerIdx + 1);
                return layersBelow.map((layer, idx) => (
                  <div key={idx} className="bg-gray-100 border border-gray-200 rounded py-2 px-3 text-center">
                    <div className="text-sm font-semibold text-gray-700">{formatCompact(layer.limit)}</div>
                  </div>
                ));
          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-50 text-xs text-gray-400 uppercase tracking-wide">
              <tr>
                ...
                <th className="px-4 py-2 text-center font-semibold">{isExcess ? 'Attach' : 'Retention'}</th>
            ...
            <tbody className="divide-y divide-gray-100">
              {[...tower].sort((a, b) => (b.attachment || 0) - (a.attachment || 0)).map((layer, idx) => {
                const isCMAI = layer.carrier?.toUpperCase().includes('CMAI');
                const layerPremium = layer.premium || 0;
                const layerRpm = layer.limit ? Math.round(layerPremium / (layer.limit / 1_000_000)) : null;
                const baseLayerPremium = tower[0]?.premium || tower.find(l => l.carrier?.toUpperCase().includes('CMAI'))?.premium || 1;
                const ilf = baseLayerPremium > 0 ? Math.round((layerPremium / baseLayerPremium) * 100) : null;
                const layerAttachment = calculateAttachment(tower, tower.indexOf(layer));
                ...
                return (
                  <tr key={idx} className={isCMAI ? 'bg-purple-50' : ''}>
                    ...
                    <td className="px-4 py-3 text-center text-gray-500">
                      {isExcess ? `xs ${formatCompact(layerAttachment)}` : formatCompact(layer.retention || retention)}
                    </td>
